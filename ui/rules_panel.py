...
_SLIDER_DEFS = [
    ...
    ("boredom", "pitch_up_th",     "Pitch Up Th (deg)",     5.0, 35.0,
     "Kepala mendongak > ini = mulai sinyal Boredom dari pitch. Naik → butuh mendongak lebih jauh → Boredom ↓ untuk kepala agak naik. Turun → kepala sedikit naik sudah → Boredom ↑."),
    ("boredom", "pitch_up_range",  "Pitch Up Range (deg)",  5.0, 40.0,
     "Rentang pitch di atas threshold sampai sinyal penuh. Naik = transisi Boredom lebih pelan/gradual saat mendongak."),
    ...
    ("boredom", "squint_blink_correction", "Squint->Blink Correction", 0.0, 1.0,
     "Koreksi blink_avg yang terinflasi akibat sipit. Naik = sipit lebih banyak dikoreksi sebelum dihitung → Boredom ↓ pada siswa fokus bermata sipit. Turun = koreksi minimal, sipit bisa inflasi blink_avg."),
    ...
    ("boredom", "chin_bore_th",     "Chin Bore Th",      0.0,  0.6,
     "Proporsi titik tangan di zona dagu/pipi minimum sebelum 'menopang dagu' dihitung sebagai boredom. Naik = butuh tangan lebih kuat di dagu sebelum Boredom ↑."),
    ("boredom", "chin_bore_range",  "Chin Bore Range",   0.1,  0.6,
     "Rentang di atas threshold sampai boost penuh. th+range = saturasi chin_bore. Naik = transisi Boredom lebih pelan."),
    ("boredom", "chin_bore_max",    "Chin Bore Max",     0.0,  1.0,
     "Kontribusi maksimal menopang dagu ke Boredom (saat mulut tertutup). Naik → Boredom ↑ dari tangan di dagu. Turun → hanya gaze/blendshape yang mempengaruhi."),
    ("boredom", "chin_jaw_closed_dz", "Chin Jaw Closed DZ", 0.0, 0.20,
     "jawOpen < ini = mulut 'tertutup' → chin boost Boredom penuh. Naik = toleransi lebih besar sebelum dianggap mulut terbuka."),
    ("boredom", "chin_jaw_open_th",   "Chin Jaw Open Th",   0.05, 0.40,
     "jawOpen > ini = mulut 'terbuka' (bicara/aktif) → chin TIDAK boost Boredom. Turun = threshold lebih sensitif (sedikit terbuka sudah dianggap bicara → Boredom ↓)."),
    ...
    ("engagement", "hand_chin_eng_pen", "Chin Eng Penalty",     0.0,  1.0,
     "Penalti engagement dari tangan di dagu saat mulut TERTUTUP (postur pasif). Naik → Engagement ↓ lebih kuat saat menopang dagu. Turun → lebih toleran."),
    ("engagement", "chin_jaw_closed_dz",    "Chin Jaw Closed DZ",   0.0, 0.20,
     "jawOpen < ini = mulut 'tertutup' → penalti chin-E penuh. Sama dengan boredom counterpart. Default: 0.08."),
    ("engagement", "chin_jaw_open_th",      "Chin Jaw Open Th",     0.05, 0.40,
     "jawOpen > ini = mulut 'terbuka' → penalti chin-E dikurangi. Naik = butuh buka lebih lebar sebelum penalti berkurang → Engagement lebih susah naik."),
    ("engagement", "chin_jaw_open_pen_reduce", "Chin Open Pen Reduce", 0.0, 1.0,
     "Fraksi pengurangan penalti chin saat jaw terbuka. 0.80 = hanya 20% penalti tersisa → Engagement ↑ saat bicara + tangan di dagu. 0 = penalti tidak berubah."),
    ("engagement", "chin_mouth_open_boost", "Chin Mouth Open Boost", 0.0, 0.5,
     "Boost Engagement saat tangan di dagu + mulut terbuka (bicara/aktif). Naik → Engagement ↑ lebih kuat saat bicara + chin. Mencerminkan aturan: ada tangan + mulut kelihatan = F+E."),
    # CONFUSION
    ...
    ("frustration", "brow_dn_th",      "BrowDown Th",     0.1,  0.6,
     "Pembagi browDown (kerutan alis). Turun = alis turun sedikit sudah → Frustration ↑. Naik = butuh ekspresi alis lebih intens."),
    ("frustration", "nose_sneer_th",   "NoseSneer Th",    0.05, 0.5,
     "Pembagi noseSneer (hidung mengernyit). Turun = gerakan hidung kecil sudah → Frustration ↑. Naik = perlu ekspresi lebih nyata."),
    ("frustration", "cheek_squint_th", "CheekSquint Th",  0.1,  0.6,
     "Pembagi cheekSquint (pipi tegang). Turun = pipi sedikit menyempit sudah → Frustration ↑. Naik = perlu pipi lebih terang-terangan tegang."),
    ("frustration", "mouth_press_th",  "MouthPress Th",   0.1,  0.6,
     "Pembagi mouthPress (bibir tertekan rapat). Turun = bibir sedikit tertekan sudah → Frustration ↑. Naik = perlu tekanan bibir lebih kuat."),
    ("frustration", "mouth_frown_th",  "MouthFrown Th",   0.05, 0.5,
     "Pembagi mouthFrown (sudut mulut turun/cemberut). Turun = cemberut sedikit sudah → Frustration ↑. Naik = perlu ekspresi kecewa lebih jelas."),
    ("frustration", "eye_squint_th",   "EyeSquint Th",    0.1,  0.6,
     "Pembagi eyeSquint (mata menyipit). Turun = mata sedikit sipit sudah → Frustration ↑. Naik = perlu sipit lebih intens."),
    ("frustration", "jaw_start",       "Jaw Start",       0.0,  0.3,
     "jawOpen minimum sebelum berkontribusi ke frustrasi. Naik = butuh gigi sedikit terbuka. Turun = kontribusi jaw aktif lebih dini."),
    ("frustration", "jaw_range",       "Jaw Range",       0.05, 0.5,
     "Rentang jawOpen di atas start sampai kontribusi penuh. Naik = transisi frustrasi dari jaw lebih pelan/gradual."),
    ("frustration", "face_weight",     "Face Weight",     0.1,  0.8,
     "Skala maks kontribusi ekspresi wajah ke Frustration. Naik → Frustration lebih tinggi dari ekspresi wajah saja (tanpa tangan). Turun → wajah kurang berpengaruh, tangan lebih dominan."),
    ("frustration", "hand_weight",     "Hand Weight",     0.3,  1.0,
     "Bobot tangan (dahi) dalam fusion Frustration. Naik → tangan di dahi lebih kuat memicu Frustration ↑. Turun → tangan di dahi perlu dikombinasikan dengan ekspresi wajah yang kuat."),
    # HYBRID
